...
@dataclass
class Signal:
    coin: str               # FIX: added — needed to identify source in signals.csv
    side: str
    entry_price: float
    stop_price: float
    tp_price: float
...
class AdaptiveSignalEngine:
    ...
    def _build_feature_frame(self, df_ohlcv: pd.DataFrame, min_len: int = 200) -> pd.DataFrame:
        ...
        try:
            df = add_features(df)
            df = build_structure(df, lookback=3)
            df = add_smc_zones(df)
    ...
    def _score_sentiment_context(self, sentiment: PerpSentimentSnapshot) -> Tuple[float, List[str]]:
        score = 0.0
        notes: List[str] = []
        ...
        if fr < 0:
            score += 0.05
            notes.append("neg_funding_tilt_long")
        elif fr > 0:
            score -= 0.05
            notes.append("pos_funding_tilt_short")
    ...
    def _build_continuation_signal(
        ...
        # DATA-DRIVEN: 55% of signals fired with htf=chop
        # Continuation needs directional conviction — hard block in chop
        if htf_regime == "chop":
        ...
        sent_score, sent_notes = self._score_sentiment_context(sentiment)
    ...
    def _build_reversal_signal(
        ...
        # DATA-DRIVEN: reversal|htf_chop|macro_down had 0% win rate across 12 trades
        # Catching bounces with no HTF direction in a macro downtrend = knife-catching
        ...
        # FIX: raised flow threshold from 0.15 → 0.25 to reduce knife-catching
        # in a bear HTF — need stronger flow evidence to go against the trend
    ...
    def _quality_filter(
        ...
        # FIX: block unknown vol_state — means atr_14 or close is 0/missing
        # prevents downstream division-by-zero in _build_trade_levels
    ...
    def _build_trade_levels(
        ...
        # FIX: use 3-bar rolling low/high instead of current bar only
        # prevents oversized stops on large engulfing candles
        # and too-tight stops on doji bars
        n = len(df)
        ...
        # Re-normalize TP to maintain minimum 2R
    ...
    def generate_signal(
        ...
        if htf_regime   == "chop": effective_threshold += 0.05
        if macro_regime == "chop": effective_threshold += 0.03
        ...
        # FIX: pass df so _build_trade_levels can use 3-bar rolling low/high
    ...
    def generate_swing_signal(
        ...
        # FIX: incorporate flow + sentiment into swing score
        flow_score, flow_notes   = self._score_flow_context(flow_snapshot)
        sent_score, sent_notes   = self._score_sentiment_context(sentiment)
        ...
        # FIX: pass df_feat so _build_trade_levels uses 3-bar rolling low/high
        stop, tp, atr_val, stop_dist, tp_dist = self._build_trade_levels(
